...
class Rule {
  ...
  constructor(chain, opts) {
    ...
    this._cssExclude = makeCssExclude(opts);
  }
  ...
  rule(rule, opts) {
    ...
    const { css: cssRuleOptions, postcss: postcssRuleOptions, usePoststylus } = options;
  ...
  useStylus() {
    const { stylus: stylusRuleOptions, modules: cssModules, modulesWithAffix } = this.options;
